...
function rayFurniture( obj ) 
{
	...
	return { id : (floor) ? floor.userData.id : null, obj : floor };
}
...
function createEstimateJson()
{
	...
	for ( var i = 0; i < room.length; i++ )
	{
		...
		var arrO = estimateFurnitures(furnitures, room[i].userData.id);
	...
	var arrO = estimateFurnitures(furnitures, null);	
...
// получаем объекты для сметы
function estimateFurnitures(furnitures, id_room)
{
	var arrO = [];
	
	for ( var i = 0; i < furnitures.length; i++ )
	{
		if(furnitures[i].id_room == id_room)
		{
			var n = arrO.length;
			...
			if(furnitures[i].obj.userData.obj3D.size) { arrO[n].size = furnitures[i].obj.userData.obj3D.size; }
			
			if(furnitures[i].obj.userData.obj3D.lotid == 72184)
			{
				var childs = getAllChildrenObj(furnitures[i].obj.children[0], []);
				var material = [];
				
				for ( var i2 = 0; i2 < childs.length; i2++ )
				{
					if(!childs[i2].obj.userData.material) continue;
					
					material[material.length] =  
					{
						id : 0, 
						group : "Material",
						name : childs[i2].obj.userData.material.containerID,
						lotId : Number(childs[i2].obj.userData.material.lotid), 
						caption : childs[i2].obj.userData.material.caption			
					};
				}

				if(material.length > 0) { arrO[n].material = material; }
			}			
		}
	}
