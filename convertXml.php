...
for ($i = 0; $i < count($array['points']); $i++) 
{
	...
	$v5->appendChild($xml->createElement('id'))->appendChild($xml->createTextNode($array['points'][$i]['id']));
	$v6 = $v5->appendChild($xml->createElement('pos'));
	$v6->appendChild($xml->createElement('x'))->appendChild($xml->createTextNode( floatToInt($array['points'][$i]['pos']['x']) )); 
	$v6->appendChild($xml->createElement('z'))->appendChild($xml->createTextNode( floatToInt($array['points'][$i]['pos']['z']) ));
...
for ($i = 0; $i < count($array['walls']); $i++) 
{
	...
	$v8->appendChild($xml->createElement('pointStart'))->appendChild($xml->createTextNode($array['walls'][$i]['pointStart']));
	$v8->appendChild($xml->createElement('pointEnd'))->appendChild($xml->createTextNode($array['walls'][$i]['pointEnd']));
	...
	if(empty($array['walls'][$i]['startShift'])) 
	{ 
	...
	{
		$v10 ->appendChild($xml->createElement('x'))-> appendChild($xml->createTextNode( floatToInt($array['walls'][$i]['startShift']['x']) )); 
		$v10 ->appendChild($xml->createElement('z'))-> appendChild($xml->createTextNode( floatToInt($array['walls'][$i]['startShift']['z']) ));
		$v11 ->appendChild($xml->createElement('x'))-> appendChild($xml->createTextNode( floatToInt($array['walls'][$i]['startShift']['x']) ));
		$v11 ->appendChild($xml->createElement('z'))-> appendChild($xml->createTextNode( floatToInt($array['walls'][$i]['startShift']['z']) ));		
	} 
	...
	for ($i2 = 0; $i2 < count($mat); $i2++)
	{
		...
		$v10->appendChild($xml->createElement('lot'))->appendChild($xml->createElement('id'))->appendChild($xml->createTextNode($mat[$i2]['lot']['id']));
		
		$v11 = $v10->appendChild($xml->createElement('matMod'));
		$matMod = $mat[$i2]['matMod'];
		...
		$v12->appendChild($xml->createElement('r'))->appendChild($xml->createTextNode( floatToInt($matMod['colorsets'][0]['color']['r']) ));
		$v12->appendChild($xml->createElement('g'))->appendChild($xml->createTextNode( floatToInt($matMod['colorsets'][0]['color']['g']) ));
		$v12->appendChild($xml->createElement('b'))->appendChild($xml->createTextNode( floatToInt($matMod['colorsets'][0]['color']['b']) ));
		$v12->appendChild($xml->createElement('a'))->appendChild($xml->createTextNode( floatToInt($matMod['colorsets'][0]['color']['a']) ));
		
		$v12 = $v11->appendChild($xml->createElement('texScale'));
		$v12->appendChild($xml->createElement('x'))->appendChild($xml->createTextNode( floatToInt($matMod['texScal']['x']) ));
		$v12->appendChild($xml->createElement('y'))->appendChild($xml->createTextNode( floatToInt($matMod['texScal']['y']) ));
		
		if(isset($matMod['texOffset']))
		{
			$v12 = $v11->appendChild($xml->createElement('texOffset'));
			$v12->appendChild($xml->createElement('x'))->appendChild($xml->createTextNode( floatToInt($matMod['texOffset']['x']) ));
			$v12->appendChild($xml->createElement('y'))->appendChild($xml->createTextNode( floatToInt($matMod['texOffset']['y']) ));			
		}
		
		if(isset($matMod['mapingRotate']))
		{
			$v11->appendChild($xml->createElement('mapingRotate'))->appendChild($xml->createTextNode( floatToInt($matMod['mapingRotate']) ));
		}		
	}	
}


if(count($array['furn']) > 0){ $v7 = $v3->appendChild($xml->createElement('furn')); }

for ($i = 0; $i < count($array['furn']); $i++)
{
	$v10 = $v7->appendChild($xml->createElement('PFurniture'));
	$v10->appendChild($xml->createElement('id'))->appendChild($xml->createTextNode($array['furn'][$i]['id']));
	$v10->appendChild($xml->createElement('lotid'))->appendChild($xml->createTextNode($array['furn'][$i]['lotid']));
	
	$v11 = $v10->appendChild($xml->createElement('pos'));
	$v11->appendChild($xml->createElement('x'))->appendChild($xml->createTextNode( floatToInt($array['furn'][$i]['pos']['x']) ));  
	$v11->appendChild($xml->createElement('y'))->appendChild($xml->createTextNode( floatToInt($array['furn'][$i]['pos']['y']) ));
	$v11->appendChild($xml->createElement('z'))->appendChild($xml->createTextNode( floatToInt($array['furn'][$i]['pos']['z']) ));
		
	$v11 = $v10->appendChild($xml->createElement('overFloor'))->appendChild($xml->createTextNode( floatToInt($array['furn'][$i]['pos']['y']) ));
	
	$v11 = $v10->appendChild($xml->createElement('rot'));
	$v11->appendChild($xml->createElement('x'))->appendChild($xml->createTextNode( floatToInt($array['furn'][$i]['rot']['x']) ));
	$v11->appendChild($xml->createElement('y'))->appendChild($xml->createTextNode( floatToInt($array['furn'][$i]['rot']['y']) ));
	$v11->appendChild($xml->createElement('z'))->appendChild($xml->createTextNode( floatToInt($array['furn'][$i]['rot']['z']) ));
	
	$v11 = $v10->appendChild($xml->createElement('size')); 
	$v11->appendChild($xml->createElement('x'))->appendChild($xml->createTextNode( floatToInt($array['furn'][$i]['size']['x']) ));
	$v11->appendChild($xml->createElement('y'))->appendChild($xml->createTextNode( floatToInt($array['furn'][$i]['size']['y']) ));
	$v11->appendChild($xml->createElement('z'))->appendChild($xml->createTextNode( floatToInt($array['furn'][$i]['size']['z']) ));

	
	$v11 = $v10->appendChild($xml->createElement('colors'));
	
	$mat = $array['furn'][$i]['colors'];
	...
	for ($i2 = 0; $i2 < count($mat); $i2++)
	{
		...
		$v12->appendChild($xml->createElement('lot'))->appendChild($xml->createElement('id'))->appendChild($xml->createTextNode($mat[$i2]['lot']['id']));		
		...
		$v14->appendChild($xml->createElement('r'))->appendChild($xml->createTextNode( floatToInt($matMod['colorsets'][0]['color']['r']) ));
		$v14->appendChild($xml->createElement('g'))->appendChild($xml->createTextNode( floatToInt($matMod['colorsets'][0]['color']['g']) ));
		$v14->appendChild($xml->createElement('b'))->appendChild($xml->createTextNode( floatToInt($matMod['colorsets'][0]['color']['b']) ));
		$v14->appendChild($xml->createElement('a'))->appendChild($xml->createTextNode( floatToInt($matMod['colorsets'][0]['color']['a']) ));
		
		$v14 = $v13->appendChild($xml->createElement('texScale'));
		$v14->appendChild($xml->createElement('x'))->appendChild($xml->createTextNode( floatToInt($matMod['texScal']['x']) ));
		$v14->appendChild($xml->createElement('y'))->appendChild($xml->createTextNode( floatToInt($matMod['texScal']['y']) ));
		
		
		if(isset($matMod['texOffset']))
		{
			$v14 = $v13->appendChild($xml->createElement('texOffset'));
			$v14->appendChild($xml->createElement('x'))->appendChild($xml->createTextNode( floatToInt($matMod['texOffset']['x']) ));
			$v14->appendChild($xml->createElement('y'))->appendChild($xml->createTextNode( floatToInt($matMod['texOffset']['y']) ));			
		}		
		
		if(isset($matMod['mapingRotate']))
		{
			$v13->appendChild($xml->createElement('mapingRotate'))->appendChild($xml->createTextNode( floatToInt($matMod['mapingRotate']) ));
		}		
	}	
}
...
if(count($array['rooms']) > 0){ $v7 = $v3->appendChild($xml->createElement('rooms')); }

for ($i = 0; $i < count($array['rooms']); $i++)
{
	$v10 = $v7->appendChild($xml->createElement('PRoom'));
	$v10->appendChild($xml->createElement('id'))->appendChild($xml->createTextNode($array['rooms'][$i]['id']));
	$v10->appendChild($xml->createElement('name'))->appendChild($xml->createTextNode($array['rooms'][$i]['name']));
	$v10->appendChild($xml->createElement('roomSType'))->appendChild($xml->createTextNode( $array['rooms'][$i]['roomSType'] ) );  
	
	$v11 = $v10->appendChild($xml->createElement('pointid'));
	
	for ($i2 = 0; $i2 < count($array['rooms'][$i]['pointid']); $i2++)
	{
		$v11->appendChild($xml->createElement('int'))->appendChild($xml->createTextNode($array['rooms'][$i]['pointid'][$i2])); 
	}
	
	if($array['rooms'][$i]['plinthLot'] != -1)
	{
		$v11 = $v10->appendChild($xml->createElement('plinthLot'));
		$v11->appendChild($xml->createElement('id'))->appendChild($xml->createTextNode($array['rooms'][$i]['plinthLot']['id']));
	}
	
	$v11 = $v10->appendChild($xml->createElement('colors'));
	
	$mat = $array['rooms'][$i]['colors'];
