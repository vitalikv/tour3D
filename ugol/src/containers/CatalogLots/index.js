...
class CatalogLots extends React.Component {
  ...
  handleClick(type, lotGroup, typeOnPlan, id, name, preview, price, caption, size, modifiers, allModifiers, data) {
    const source = {
    ...
    reactGlobal.sendMessage('CATALOG.OBJECT_PICK', {
      type: type,
      lotGroup: lotGroup,
      size: size,
      modifiers: modifiers,
      allModifiers: allModifiers,
      typeOnPlan: typeOnPlan,
      id: id,
      name: name,
      preview: preview,
      source: source,
      replace: reactGlobal.replaceMode,
      data: data
    });
    this.closeModal();
  }
  ...
  handleApply(type, lotGroup, typeOnPlan, id, name, image, data) {
    // console.log(type, id, name, image)
    this.openModal('layoutAlert', { type: type, id: id, name: name, image: image, data: data })
  }
