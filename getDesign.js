...
function getDesignFile(cdm) 
{  
	select_room = null;
	select_room = clickO.last_obj; 	
	if(!select_room) { select_room = clickO.obj; }
	...
	({
		...
		{   
			...
			{	
				...
				if(cdm.type) { var cdm2 = { type : 'planoplan', lotid : cdm.lotid } }
				else { var cdm2 = {} }  
				getDesignJson(json, cdm.room_id, roomID, cdm2);	
			} 
			catch (err) 
			{
				// если не json, то это xml
				$.ajax
				({
					url: 'convertArr.php',
					type: 'POST',
...
{  
	...
	{ 		
		arrWallFront.wall = [{ obj : room[id].w[i], index : (room[id].s[i] == 0) ? 2 : 1 }]; 
...
function getDesignJson(arr, id_room, roomID, cdm)
{
	console.log(arr, id_room, roomID, cdm);
	
	if(arr.code_server != 200) 
	{ 
		emitAction('load_error', { code: arr['code_server'] }); 
		console.log('server : ' + arr.code_server);

		sendMessage('EDITOR.DESIGN_PICK_ERROR');
		return; 
	}
	...
	if(cdm.type)
	{
		if(cdm.type == 'planoplan')
		{
			for ( var i = 0; i < arr.floors[0].rooms.length; i++ )
			{
				if(Number(id_room) == Number(arr.floors[0].rooms[i].roomSType)){ id_room = arr.floors[0].rooms[i].id; break; }
			}
		}
	}
	
	if(cdm.lotid) { UIInvokeFunction('DesignProjectLoadStart', cdm.lotid); }
	...
	if(points.length == 0) { sendMessage('EDITOR.DESIGN_PICK_ERROR'); return; }
	...
	for ( var i = 0; i < roomW.length; i++ )
	{
		...
		loadPopObj_1([{ obj: roomW[i], lotid: walls[n].material.lotid, start: 'load', index : index, color : walls[n].material.color, scale : walls[n].material.scale }]);
	}
	...
	for ( var i2 = 0; i2 < colors.length; i2++ )	
	{
		...
		loadPopObj_1([{ obj: obj, lotid: material.lotid, start: 'load', color : material.color, scale : material.scale }]);
	}	
		...
		if(objPop[i].roomId == id_room)
		{
			...
			if(obj_pop[i].colors)
			{
				...
				{						
					...
					var containerID = obj_pop[i].colors[i2].containerID;
					var lotid = obj_pop[i].colors[i2].lot.id;
					...
					{				
						if(obj_pop[i].colors[i2].matMod.colorsets[0].color)
						{
							...
							var color = new THREE.Color('rgb('+color.r+'%,'+color.g+'%,'+color.b+'%)');
							var opacity = color.a;
						}
						
						if(obj_pop[i].colors[i2].matMod.texScal)
						{
							var scale = new THREE.Vector2( obj_pop[i].colors[i2].matMod.texScal.x, obj_pop[i].colors[i2].matMod.texScal.y );
						}				
					}				
					
					material[material.length] = { lotid : lotid, containerID : containerID, color : color, opacity : opacity, scale : scale } 
				}					
			}			
	...
	if(cdm.lotid) { UIInvokeFunction('DesignProjectLoadEnd', cdm.lotid); }
	sendMessage('EDITOR.DESIGN_PICK_SUCCESS');
}
