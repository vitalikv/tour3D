...
if(isset($xml["floors"]["PFloor"][0])) { $arrFloor = $xml["floors"]["PFloor"][0]; }
else { $arrFloor = $xml["floors"]["PFloor"]; }


if($arrFloor["points"])
{
	$point = $arrFloor["points"]["PPoint"];
...
if($arrFloor["furn"])
{
	...
	for ($i = 0; $i < count($obj); $i++)
	{
		...
		for ($i2 = 0; $i2 < count($psurface); $i2++)
		{
			...
			if(isset($color))
			{
				$arr["furn"][$i]["colors"][$i2]["matMod"]["texOffset"]["x"] = (isset($color["x"])) ? intToFloat($color["x"]) : 0; 
				$arr["furn"][$i]["colors"][$i2]["matMod"]["texOffset"]["y"] = (isset($color["y"])) ? intToFloat($color["y"]) : 0; 
			}			
			...
			if(isset($color))
			{
				$arr["furn"][$i]["colors"][$i2]["matMod"]["mapingRotate"] = intToFloat($color); 
			}			
		}		
	}	
}
...
$arr["height"] = intToFloat($arrFloor["height"]);
$arr["id"] = $arrFloor["id"];
