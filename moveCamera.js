...
{
	if(param_ugol.start) if(UI.activeInput) { return; }
	
	if(clickO.last_obj) if(clickO.last_obj.userData.tag == 'obj') return;
...
function cameraMove3D( event )
{
	...
	{
		if(infProject.scene.type.startMoveCamera)
		{ 
			if(isMouseDown2 || isMouseDown3)
			{ 
				infProject.scene.type.startMoveCamera = false;
				UIInvokeFunction('CameraMove');				
			}
		}
...
{ //return; 
	for ( var i = 0; i < wallVisible.length; i++ )
	{
		...
		var dir = new THREE.Vector3();
		if ( camera.getWorldDirection(dir).dot( wallVisible[ i ].normal.clone() ) > 0 )  
		{
			wall.visible = false;
			if(wall.userData.wall.outline) { deleteOutline(); }
...
{				 
	...
	if(camera == cameraWall) detectArrayPointToScreen();
}
...
function cameraZoomTopLoop() 
{
	...
	if ( camera == camera3D )
	{
		if ( zoomLoop == 'zoomOut' ) { cameraZoom3D( 0.3 ); flag = true; }
		if ( zoomLoop == 'zoomIn' ) { cameraZoom3D( -0.3 ); flag = true; }
	}
	if ( camera == cameraWall )
	{
		...
		if(flag) camera.updateProjectionMatrix();  
	}
...
function cameraZoomTop( delta )
{
	...
	if(typeUI == 'planoplan') 
	{ 
		var value = ratioDifferenceNumber({ x1: 0.5, x2: 20, y1: 1, y2: 0, t : delta});  
		
		if(value < 0 || value > 1) return;
		//console.log(value);
		UIInvokeFunction('SetCameraZoom', value ); 
	}
...
// центрирование камеры при загрузке проекта 
function centerStartCamera3D()
{
	var pos = new THREE.Vector3();
	...
	camera3D.position.set(pos.x + 5, 7, pos.z + 5 );
	camera3D.lookAt(new THREE.Vector3( pos.x, 0, pos.z ));
	centerCam.set(pos.x, 0, pos.z);
}
...
function previewRender360()
{
	if(!camera3D.userData.camera.rot360.start) return;
	if(camera != camera3D) return;
	
	
	if(!camera3D.userData.camera.rot360.qEnd)
	{
		camera3D.userData.camera.rot360.qEnd = new THREE.Quaternion().setFromEuler(new THREE.Euler(0, camera3D.rotation.y, 0)).normalize();		
	}
	
	var x = Math.abs(camera3D.rotation.x);
	var z = Math.abs(camera3D.rotation.z);
	
	if(x > 0.01 || z > 0.01)
	{
		camera3D.quaternion.slerp(camera3D.userData.camera.rot360.qEnd, 0.1); 
	}
	else
	{
		var step = Math.PI * 2/200;

		camera3D.userData.camera.rot360.angle += step;
		camera.rotateOnWorldAxis(new THREE.Vector3(0,1,0), step);	
		
		if(camera3D.userData.camera.rot360.angle >= Math.PI * 2) { camera3D.userData.camera.rot360 = { start : false, angle : 0, qEnd : null } }		 	
	}
