...
{	
	// если кликнули на окно, которое не относится к фронтальной стене, то ничего не делаем
	...
	if(camera == cameraWall || camera == cameraTop) { obj_selected = obj; }
...
{			
	...
	{ 
		...
		UI('door_width_1').val(Math.round((d1 - 0.1) * 100) * 10)
		UI('door_height_1').val(Math.round((d2 - 0.1) * 100) * 10);	
		UI.showToolbar('door-2d-toolbar');  
		
		if(wd.userData.door.topMenu) { UI('door-params').val('edit'); }
		else { UI('door-params').val('info'); } 
	}
	else if(menu == 'window')
	{		
...
{   
	var str = (wd.userData.tag == 'window') ? 'window' : 'door';
	
	var wall = wd.userData.door.wall;
	
	var x = Number(UI(str+'_width_1').val()) / 1000;	 			// ширина 	
	var y = Number(UI(str+'_height_1').val()) / 1000;  				// высота  
	...
	{
		var h = (y - Math.abs( wd.geometry.boundingBox.max.y - wd.geometry.boundingBox.min.y )) / 2;    // вычитаем изменение высоты окна/двери  
...
function setPosDoorLeaf_2(door) 
{
	...
	if(door.userData.door.mirror)
	{
		popObj.scale.x = -popObj.scale.x; 
		popObj.scale.z = -popObj.scale.z; 		
	}
}
...
function clickWDMouseUp(wd)
{
	...
	if(camera == cameraWall) 
	{ 
		// обновляем плинтуса, если перетащили wd
