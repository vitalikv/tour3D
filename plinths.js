...
{	
	...
	for (i = 0; i < room.w.length; i++) 
	{
		...
		for (i2 = 0; i2 < arrPos.length; i2++)
		{
			...
			var n = plinth.obj.length;
			plinth.obj[n] = obj;
			plinth.wall[n] = room.w[i];			
...
function updatePlinthsReplaceDoor(obj)  
{
	...
	{	
		if(arrWallFront.room)
		{
			updateFormPlinths( [arrWallFront.room] );
			hidePlinthsCameraWall(arrWallFront.room.userData.room.plinth);
		}		
	}
...
// удаляем плинтуса в комнатах и создаем новые (тем самыс производим обновление)
...
// скрываем все плинтуса, которые не должны показываться в cameraWall
function hidePlinthsCameraWall(plinth) 
{
	if(!plinth.o) return; 
	
	for ( var i = 0; i < plinth.obj.length; i++ )
	{
		plinth.obj[i].visible = false;
		
		// отображаем плинтуса у фронтальных стен
		for ( var i2 = 0; i2 < arrWallFront.wall.length; i2++ )
		{
			if(plinth.wall[i] == arrWallFront.wall[i2].obj) { plinth.obj[i].visible = true; break; }
		}
		
		// отображаем плинтуса у соседних стен
		for ( var i2 = 0; i2 < arrWallFront.wall_2.length; i2++ )
		{
			if(plinth.wall[i] == arrWallFront.wall_2[i2]) { plinth.obj[i].visible = true; break; }
		}		
	}
}


// показываем все плинтуса после выхода из cameraWall в camera3D
function showPlinthsCamera3D(visible)
{
	for ( var i = 0; i < room.length; i++ )
	{
		if(!room[i].userData.room.plinth.o) continue;
		
		var plinths = room[i].userData.room.plinth.obj;
		
		for ( var i2 = 0; i2 < plinths.length; i2++ )
		{
			plinths[i2].visible = visible;
		}		
	}	
}
