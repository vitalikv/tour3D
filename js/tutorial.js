var tutorial = (function () {
  ...
  function showTutorial(name, page, target, callback, isNext) {
    ...
    template.find('.tutorial__close').on('click touchstart', function () { hideTutorial(template); });
    ...
    function initNextButton() {
      if (tutorial.length > page + 1) {
        template.find('.tutorial__btn').on('click touchstart', function () {
    ...
    function createTemplateNav() {
      ...
      for (var i = 0; i < tutorial.length; i++) {
        ...
        if (i == page)
          el.addClass('_active');
        (function (i) {
          el.on('click touchstart', function () { hideTutorial(template); showTutorial(name, i, target, false, true); });
        })(i);
        template.find('.tutorial__nav').append(el);
      }
      template.data('page', page);
    }
    $('.tutorial__disable-ui, .tutorial').on('mousedown mousemove touchend touchmove', function(e) {
      e.stopPropagation();
    });
